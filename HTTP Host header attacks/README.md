...
Change the host header to get the logs of password reset link to the attacker controlled server when sending the password reset link
...
In scan we got the host header SSRF with the collaborator payload. perfomed intruder attack to identify the internal IP address to access the admin panal with the given IP range of addresses
